...
function Header() {
  ...
  return (
    //         justify-between: "Spread out from left to right!"
    //         items-center: "Line up in the middle vertically!" // center in y axis
    //          border: gives a border for clarity
    // we can add fixed w-full top-0 for good fixed header
    <nav className=" bg-gray-900  text-white text-lg flex justify-between px-12 items-center h-20">
      {/* <h1>Header</h1> */}
      <Link to="/"><svg class="VXJlj" viewBox="0 0 61 61" height="49" width="49">
        <g clip-path="url(#a)">
          <path
            fill="#FF5200"
      ...
      <ul className="flex">
        ...
        <div className="flex mx-4">
          ...
          {/* <li className="px-2">LOGIN</li>  -- old code */}
          {/* Below is new code */}
          <li className="px-2 cursor-pointer" onClick={openModal}>LOGIN</li>
        </div>
        <div className="flex mx-4">
          <span className="mt-1">
            <IoCart />
          </span>
          <Link to="/cart"><li className="px-2 cursor-pointer">CART - {cartItems.length}</li></Link>
        </div>
      </ul>
      {/* Adding Login component here */}
